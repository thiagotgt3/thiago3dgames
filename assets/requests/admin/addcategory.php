                        ...
                        if (strlen($category_name) <= 100) {
                            ...
                            if (isset($_FILES['__game_image']['tmp_name'])) {
                                  ...
                                  if ($_FILES['__game_image']['size'] > 1024) {
                                        ...
                                        $GameMonetizeConnect->query("INSERT INTO ".CATEGORIES." (category_pilot, name, image) VALUES ('{$seo_name}','{$category_name}', '{$addgame_mediaurl}')");
                                                
                                    } else { $data['error_message'] = $lang['error_image_size']; }
                                    } else { $data['error_message'] = $lang['message_select_img_files']; }
                            }
                            else {
                                  $GameMonetizeConnect->query("INSERT INTO ".CATEGORIES." (category_pilot, name) VALUES ('{$seo_name}','{$category_name}')");
                            }
