                ...
                if ($editgame['import'] == 0) {
                    if (!empty($_POST['eg_image'])) {
                        if (!empty($_POST['eg_file'])) {
                            ...
                            $GameMonetizeConnect->query("UPDATE ".GAMES." SET name='{$editgame['name']}', game_name='{$editgame['game_name']}', image='{$editgame_mediaurl}', import='0', category='{$editgame['category']}', description='{$editgame['description']}', instructions='{$editgame['instructions']}', file='{$editgame['file']}', game_type='{$editgame['type']}', w='{$editgame['width']}', h='{$editgame['height']}', rating='{$editgame['rating']}', featured_sorting='{$editgame['sorting']}' WHERE game_id='{$editgame['id']}'");
                        ...
                        if ($_FILES['__eg_image']['size'] > 1024) {
                            ...
                            if (true) {
                                if(true) {
                                   ...
                                   $GameMonetizeConnect->query("UPDATE ".GAMES." SET name='{$editgame['name']}', game_name='{$editgame['game_name']}', image='{$editgame_mediaurl}', import='1', category='{$editgame['category']}', description='{$editgame['description']}', instructions='{$editgame['instructions']}', file='{$editgame['file']}', game_type='{$editgame['type']}', w='{$editgame['width']}', h='{$editgame['height']}', rating='{$editgame['rating']}', featured_sorting='{$editgame['sorting']}' WHERE game_id='{$editgame['id']}'");
